...
function draw_circle(cr,pos_x,pos_y,rectangle_x,rectangle_y,color1,color2,color3,trans, parameter, hide)
...
function draw_ram(cr,x_pos,y_pos, radius, r, g, b, transparency)
	cairo_set_operator(cr, operator_transpose[mode])
	cairo_set_source_rgba(cr,r, g, b, transparency)
...
function draw_function(cr)
    ...
    local start_rect_y = 0
   	start_rect_height = 88
    -- Start with 172
    start_rect_width = 108
    gap_y_text = (start_rect_height/2)-7
    gap_x = 10
	gap_y = 10
	gap_x_fix = 15
   	gap_y_fix = 15
	start_rect_width_no_battery = 66.5
	radius = 10
    
	if battery then
 
		draw_circle(cr,start_rect_x+gap_x_fix,start_rect_y+gap_y_fix, start_rect_width, start_rect_height,r_battery, g_battery, b_battery, transparency_battery,conky_parse('${battery_percent}'), hide)
		draw_battery(cr,gap_x+gap_x_fix, gap_y_fix,start_rect_height,r_battery, g_battery, b_battery, transparency_battery)
		
		for i=1,drives do
			draw_circle(cr,(start_rect_x+start_rect_width+gap_x_distance)*i+gap_x_fix,start_rect_y+gap_y_fix, start_rect_width, start_rect_height,drive_colors[i][1], drive_colors[i][2], drive_colors[i][3], drive_colors[i][4],conky_parse('${fs_free_perc ' .. drive_paths[i] .. '}'), hide)
			draw_folder(cr,(start_rect_x+start_rect_width+gap_x_distance)*i+gap_x_fix,gap_y_fix,start_rect_height,drive_paths[i],drive_names[i],drive_colors[i][1], drive_colors[i][2], drive_colors[i][3], drive_colors[i][4])
		end

		draw_circle(cr,(start_rect_x+start_rect_width+gap_x_distance)*(drives+1)+gap_x_fix,start_rect_y+gap_y_fix, start_rect_width, start_rect_height,r_CPU, g_CPU, b_CPU, transparency_CPU,conky_parse('${cpu cpu0}'), hide)
		draw_cpu(cr,number_of_cpus, (start_rect_x+start_rect_width+gap_x_distance)*(drives+1)+gap_x+gap_x_fix,gap_y_fix+gap_y,r_CPU, g_CPU, b_CPU, transparency_CPU)

		draw_circle(cr,(start_rect_x+start_rect_width+gap_x_distance)*(drives+2)+gap_x_fix,start_rect_y+gap_y_fix, start_rect_width, start_rect_height,r_RAM, g_RAM, b_RAM, transparency_RAM,tostring(100-tonumber(conky_parse('${memperc}'))), hide)
		draw_ram(cr,(start_rect_x+start_rect_width+gap_x_distance)*(drives+2)+gap_x+gap_x_fix,gap_y+gap_y_fix, radius, r_RAM, g_RAM, b_RAM, transparency_RAM)

	else
	
		for i=1,drives do
			draw_circle(cr,(start_rect_x+start_rect_width_no_battery)+gap_x_fix,start_rect_y+gap_y_fix, start_rect_width, start_rect_height,drive_colors[i][1], drive_colors[i][2], drive_colors[i][3], drive_colors[i][4],conky_parse('${fs_free_perc ' .. drive_paths[i] .. '}'), hide)
			draw_folder(cr,(start_rect_x+start_rect_width_no_battery)+gap_x_fix,gap_y_fix,start_rect_height,drive_paths[i],drive_names[i],drive_colors[i][1], drive_colors[i][2], drive_colors[i][3], drive_colors[i][4])
			start_rect_width_no_battery = start_rect_width_no_battery + start_rect_x + gap_x_distance + 108
		end

		draw_circle(cr,(start_rect_x+start_rect_width_no_battery)+gap_x_fix,start_rect_y+gap_y_fix, start_rect_width, start_rect_height,r_CPU, g_CPU, b_CPU, transparency_CPU,conky_parse('${cpu cpu0}'), hide)
		draw_cpu(cr,number_of_cpus,(start_rect_x+start_rect_width_no_battery)+gap_x+gap_x_fix,gap_y_fix+gap_y,r_CPU, g_CPU, b_CPU, transparency_CPU)
		start_rect_width_no_battery = start_rect_width_no_battery + start_rect_x + gap_x_distance + 108

		draw_circle(cr,(start_rect_x+start_rect_width_no_battery)+gap_x_fix,start_rect_y+gap_y_fix, start_rect_width, start_rect_height,r_RAM, g_RAM, b_RAM, transparency_RAM,tostring(100-tonumber(conky_parse('${memperc}'))), hide)
		draw_ram(cr,(start_rect_x+start_rect_width_no_battery)+gap_x+gap_x_fix,gap_y+gap_y_fix, radius, r_RAM, g_RAM, b_RAM, transparency_RAM)		
